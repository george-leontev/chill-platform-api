...
class FriendService:
    ...
    def get_accepted_friends(self, user_id: int, page: int, size: int) -> FriendsModel:
        friends, total = self.friend_repository.get_accepted_friends(user_id, page, size)

        result = self.mapper.to_list_model(friends, total, page, size)

        return result

    def get_incoming_requests(self, user_id: int, page: int, size: int) -> FriendsModel:
        incoming, total = self.friend_repository.get_incoming_requests(user_id, page, size)

        result = self. mapper.to_list_model(incoming, total, page, size)

        return result

    def get_outgoing_requests(self, user_id: int, page: int, size: int) -> FriendsModel:
        outgoing, total = self.friend_repository.get_outgoing_requests(user_id, page, size)

        result = self.mapper.to_list_model(outgoing, total, page, size)
